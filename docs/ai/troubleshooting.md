...
is empty. Two caveats before quoting a version at a user:
...
module tag that appears in the log line. Format placeholders (`%d`, `%s`, …)
are quoted as they appear in the source, so a pasted line can be matched by
its fixed prefix.
