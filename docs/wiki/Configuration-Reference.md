...
| `daynight.history_s` | int | 14400 (4h) | 0–172800 (48h) | **Live** | Seconds of decision history kept in RAM for the WebUI tuning graph, served by `GET /control?dn_history=1`; `0` = off. One 16-byte sample per 10 s, allocated on first push and resized in place when this changes, so 4 h costs 22.5 KB and the 48 h ceiling 270 KB. Anything above the ceiling is clamped (the POST response echoes the clamped value, and the daemon logs the effective size). The daemon owns the series because the WebUI is plain HTTP and therefore has no Service Worker to collect with while the tab is hidden or closed. |
