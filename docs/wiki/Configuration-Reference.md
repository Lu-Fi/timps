...
| `daynight.history_s` | int | 0 (off) | 0–172800 (48h) | **Live** | Seconds of decision history kept in RAM for the WebUI tuning graph, served by `GET /control?dn_history=1`; `0` = off, and at `0` nothing is allocated at all. One 16-byte sample per 10 s, allocated on first push and resized in place when this changes, so 4 h costs 22.5 KB and the 48 h ceiling 270 KB. Anything above the ceiling is clamped (the POST response echoes the clamped value, and the daemon logs the effective size). The tuning page POSTs this to 4 h while it is open and back to `0` when it is closed, so a camera nobody is watching carries nothing. A POST applies live but is **never persisted** (`F_NOPERSIST`) — the page toggles it twice per visit, which must not churn flash, and a reboot is the backstop if the page's teardown never ran. Set it in `timps.conf` by hand to collect unattended across reboots; the page's "collect in background" switch keeps it running only until the next restart. The daemon owns the series because the WebUI is plain HTTP and therefore has no Service Worker to collect with while the tab is hidden or closed. |
