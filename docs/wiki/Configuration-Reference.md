...
| `webrtc.channel` | int | 0 | 0–(`MS_MAX_VSTREAM`-1) | File-only | Which video stream to send. It must be H.264 — there is no H.265 or transcoding over WebRTC. Its `video<N>.profile` decides which browsers can play the result: Chromium accepts any profile, Firefox only Baseline (`0`). |
