...
| `webrtc.channel` | int | 0 | 0–(`MS_MAX_VSTREAM`-1) | File-only | Which video stream to send when the WHEP `POST` carries no `?chn=N` (a client picks the stream per session with it). It must be H.264 — there is no H.265 or transcoding over WebRTC. Its `video<N>.profile` decides which browsers can play the result: Chromium accepts any profile, Firefox only Baseline (`0`). |
...
| `video<N>.profile` | int | 2 / 2 | 0 (baseline) – 2 (high) | Restart-only | H.264/H.265 encode profile. **Matters for WebRTC:** the WHEP answer's `profile-level-id` comes from this stream's live SPS, and Firefox offers (and accepts) H.264 as Baseline only — it refuses a Main/High answer at `setRemoteDescription()`, so the session never establishes. Chromium accepts any profile and decodes from the in-band SPS. Set `0` on the `webrtc.channel` stream if Firefox must work; the preview page detects the mismatch and disables its WebRTC option either way. See the top-level `README.md` § "WebRTC / WHEP". |
